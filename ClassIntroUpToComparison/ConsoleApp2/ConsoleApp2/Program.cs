...
namespace ConsoleApp2
{
    ...
    class Human : IComparable // This interface contains the method signature (but not the definition)
                              //    for how two objects should be compared.  See CompareTo() below for more.
    {
        ...
        // NOTE: Equals should NEVER throw on incorrect type provided
        ...
        // This method is included to satisfy the IComparable interface that we included at the top of the class.
        // Expected implementation functionality is for the two objects being compared to be considered "left"
        // and "right".  The invoking instance (this) is considered the left object, and the instance accepted
        // is considered the "right" object.
        // The method will return an interger value following these rules which will result in an ascending Sort()
        // If "left" is smaller than "right, return a negative value
        // If "left" and "right" are equal, return 0
        // If "left" is larger than "right", return a positive value
        
        // NOTE: There is no integer value that can be returned to indicate an illegal type supplied for "right",
        //          so CompareTo() will ALWAYS throw if an incorrect type is supplied.
        public int CompareTo(object obj)
        {
            if (!(obj is Human other))
                throw new ArgumentException("You have not provided a Human to Compare!");

            // The following implements a two layer sort.  If the ages are equal, then sort by First name
            int sortResult = Age.CompareTo(other.Age);
            if (sortResult == 0)
                //return other.First.CompareTo(First);
                return First.CompareTo(other.First) * -1;

            return sortResult;
        }

        public static int SortByDescendingFirstName(Human left, Human right)
        {
    ...
    {        
        static void Main(string[] args)
        {
            ...
            humans.Sort(SortByLastName);
            foreach (Human h in humans)
                Console.WriteLine(h);
            Console.WriteLine();

            humans.Sort(Human.SortByDescendingFirstName);
            foreach (Human h in humans)
                Console.WriteLine(h);
            Console.WriteLine();

            humans.Sort((left, right) =>  right.Age.CompareTo(left.Age));
            foreach (Human h in humans)
                Console.WriteLine(h);
            Console.WriteLine();
        ...
        static int SortByLastName (Human left, Human right)
        {
            if (left == null && right == null) return 0;
            if (left == null) return -1;
            if (right == null) return 1;
